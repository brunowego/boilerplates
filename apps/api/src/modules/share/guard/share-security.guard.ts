...
export class ShareSecurityGuard implements CanActivate {
  constructor(private shareService: ShareService, private prismaService: PrismaService) {}
  ...
  async canActivate(context: ExecutionContext) {
    ...
    if (share.security?.password && !shareToken)
      throw new ForbiddenException('This share is password protected', 'share_password_required')

    if (!(await this.shareService.verifyShareToken(shareId, shareToken)))
      throw new ForbiddenException('Share token required', 'share_token_required')
