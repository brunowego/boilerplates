...
export class SettingService {
  constructor(
    @Inject('CONFIG_VARIABLES') private settingVariables: Setting[],
  ...
  get(key: `${string}.${string}`): any {
    ...
    const value = settingVariable.value ?? settingVariable.default_value
  ...
  async updateMany(data: UpdateSettingDto[]) {
    ...
    for (const variable of data) {
      response.push(await this.update(variable))
    }

    return response
  }

  async update(data: UpdateSettingDto) {
