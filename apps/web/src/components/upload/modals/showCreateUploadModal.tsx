...
const showCreateUploadModal = (
  ...
  return modals.openModal({
    title: 'Share',
    children: <CreateUploadModalBody options={options} uploadCallback={uploadCallback} />,
  })
}
...
const CreateUploadModalBody = ({
  ...
  const validationSchema = yup.object().shape({
    link: yup.string().required().min(3).max(50).matches(new RegExp('^[a-zA-Z0-9_-]*$'), {
            ...
            <Col xs={3}>
              <Button
                ...
                onClick={() =>
                  form.setFieldValue(
                    'link',
                    Buffer.from(Math.random().toString(), 'utf8').toString('base64').substr(10, 7)
                  )
                }
              >
                Generate
                ...
                <Col xs={6}>
                  <Select
                    ...
                    data={[
                      {
                        value: '-minutes',
                        label: 'Minute' + (form.values.expiration_num == 1 ? '' : 's'),
                      },
                      {
                        value: '-hours',
