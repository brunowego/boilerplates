...
import { successResponseWithDataSchema } from '../users/schema'
...
export const getUsersByOrganizationIdRoute = createRoute({
    ...
    query: paginationQuerySchema.extend({
      ...
      role: z
        ...
        .openapi({
          param: {
            description:
              'Filter by role in organization (if not set, then all users are returned)',
          },
        }),
    }),
  },
  responses: {
    200: {
      ...
      content: {
        'application/json': {
          schema: successResponseWithPaginationSchema(
            apiOrganizationUserSchema,
          ),
        },
      },
