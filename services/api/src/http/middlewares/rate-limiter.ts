...
class RateLimiter extends RateLimiterMemory {
  public middleware(): MiddlewareHandler<Env> {
    return async (ctx, next) => {
      ...
      if (!ipAddr && !username) return next()
      ...
      try {
        ...
        return ctx.json({ success: false, error: 'Too many requests' }, 429)
      }
...
export const rateLimiter = (options: IRateLimiterOptions) =>
  new RateLimiter(options).middleware()
...
export const signInRateLimiter =
  (): MiddlewareHandler<Env> => async (ctx, next) => {
    const ipAddr = ctx.req.header('x-forwarded-for') || ''
