...
>(({ className, children, ...props }, ref) => (
  <ModalPortal>
    ...
    >
      ...
      <ModalPrimitive.Close className='absolute top-4 right-4 rounded-sm opacity-70 ring-offset-background transition-opacity disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2'>
        <X className='size-5' />
...
const ModalHeader = ({
  ...
  <div
    className={cn(
      'flex flex-col space-y-1.5 rounded-t-[inherit] border-b bg-secondary p-6 text-center sm:text-left',
      className,
    )}
    {...props}
  />
