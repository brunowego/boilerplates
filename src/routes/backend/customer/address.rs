...
pub async fn create_address(
    ...
    let address_new_model = address.try_into_model().map_err(|error| {
        eprintln!("[*] Error converting into model {}", error);
        AppError::new(StatusCode::INTERNAL_SERVER_ERROR, "Something went wrong")
    })?;

    Ok(Json(RespondAddress {
        id: address_new_model.id,
        country: address_new_model.country,
        state: address_new_model.state,
        town: address_new_model.town,
        zip: address_new_model.zip,
        address_line_1: address_new_model.address_line_1,
        address_line_2: address_new_model.address_line_2,
    }))
}
