...
pub fn create_jwt(jwt_secret: &str) -> Result<String, AppError> {
    ...
    encode(&Header::default(), &claims, key).map_err(|error| {
        eprintln!("Could not create jwt {}", error);
        AppError::new(
            StatusCode::INTERNAL_SERVER_ERROR,
            "Error occured when creating jwt token, Please try again later",
        )
    })
}
pub fn validate_jwt(jwt_secret: &str, token: &str) -> Result<bool, AppError> {
    ...
    decode::<Claims>(token, &key, &validation).map_err(|error| match error.kind() {
        jsonwebtoken::errors::ErrorKind::ExpiredSignature => {
            AppError::new(StatusCode::UNAUTHORIZED, "Your session has expired")
        }
        _ => {
